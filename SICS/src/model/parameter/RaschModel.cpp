...
void RaschModel::setEstimationNodes(QuadratureNodes* n) { this->nodes = n; }
...
void RaschModel::successProbability(DimensionModel *dimensionModel, QuadratureNodes * quadNodes)
{
	...
	if (typeid(*dimensionModel) == typeid(UnidimensionalModel))
	{
		if (probabilityMatrix == NULL)
			...
			probabilityMatrix = new Matrix<double>((int) q, items);
		...
		for (unsigned int k = 0; k < q; k++)
		{
			for (int i = 0; i < items; i++)
			{
				...
				(*probabilityMatrix)(k, i) = successProbability(theta_d, b_d);
			}
		}
	}
}
...
void RaschModel::setParameterSet(double*** pair) { this->parameterSet = pair; }

double*** RaschModel::getParameterSet() { return (this->parameterSet); }

double RaschModel::getProbability(int node, int item) { return ((*probabilityMatrix)(node, item)); }

double RaschModel::logLikelihood(double* args, double* pars, int nargs,	int npars)
{
	//args
	/*
	 * a = 1
...
void RaschModel::Ngradient(double* args, double* pars, int nargs, int npars, double* gradient)
{
...
void RaschModel::NHessian(double*args, double* pars, int nargs, int npars, double* hessian)
{
...
void RaschModel::gradient(double* args, double* pars, int nargs, int npars, double* gradient)
{
	/*
	 *
	 * What we need
