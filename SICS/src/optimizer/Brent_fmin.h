...
double static Brent_fmin(double * interval, double tol,
                         double (*function)(double, bool *, int, int, Model*),
                         bool * pattern, int size, int node, Model *model, int maxIters)
{
...
double static Brent_fmin(double * interval, double tol,
		double (*function)(double, bool *, int, int, Model*, double ***), bool * pattern,
		int size, int node, Model *model, double *** parSet, int maxIters)
{
	...
	d = 0.;/* -Wall */
	e = 0.;
	fx = (*function)(x, pattern, size, node, model, parSet);
	fv = fx;
	fw = fx;
	tol3 = tol / 3.;
	/* main loop starts here ----------------------------------- */
	for (;;) {
		...
		if (fabs(d) >= tol1)
			...
			u = x - tol1;
		fu = (*function)(u, pattern, size, node, model, parSet);
		/* update a, b, v, w, and x */
		if (fu <= fx) {
			if (u < x)
				b = x;
